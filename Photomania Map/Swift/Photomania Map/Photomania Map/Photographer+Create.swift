...
extension Photographer {
	...
	class func userInManagedObjectContext(context : NSManagedObjectContext) -> Photographer? {
		return self.photographer(" My Photos", context: context)
	}
	
	func isUser() -> Bool! {
		return Photographer.userInManagedObjectContext(self.managedObjectContext!) == self
	}
